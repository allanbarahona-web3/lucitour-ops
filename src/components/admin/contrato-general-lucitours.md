# CONTRATO GENERAL DE VIAJE TURISTICO (PLANTILLA UNICA)

> Nota tecnica: Esta plantilla esta pensada para motor de variables tipo `{{campo}}` y condicionales `{{#if condicion}} ... {{/if}}`.
> Si usan otro motor (Docxtemplater, Handlebars, etc.), se ajusta la sintaxis manteniendo los mismos nombres de campo.

Entre nosotros:

(a) VIAJES LUCITOURS TURISMO INTERNACIONAL SOCIEDAD ANONIMA, cedula juridica numero 3-101-874546, con domicilio social en la Provincia de Cartago, Canton Cartago, Distrito Occidental, frente a la tienda deportiva Carolina, en centro comercial, segunda planta primer local a mano izquierda Lucitours, representada en este acto por:
...
vecino de {{client.address}}, en adelante denominado como el "Cliente".

{{#if travelers.hasCompanions}}
Adicionalmente, comparecen como acompañantes del Tour:
{{#each travelers.companions}}
- {{fullName}}, identificacion ({{idType}}) numero {{idNumber}}
{{/each}}
{{/if}}
...
La representacion y firma por parte de Lucitours podra realizarse de manera conjunta o separada por sus representantes legales indicados, de conformidad con las facultades vigentes de representacion de la sociedad.
...
Equipaje permitido: 1 articulo personal de hasta 10 kg (mochila de espalda, no carry-on). Excesos de equipaje son por cuenta del Cliente.
...
# Campos que se llenan automaticamente desde el flujo de agentes

## 1) Identificacion y datos del cliente
- `{{client.fullName}}`
- `{{client.civilStatus}}`
- `{{client.profession}}`
- `{{client.nationality}}` (solo si pasaporte)
- `{{client.idType}}` (cedula/pasaporte/dimex)
- `{{client.idNumber}}`
- `{{client.address}}`
- `{{client.email}}`
- `{{client.phone}}`
...
- `{{trip.startDate}}`
- `{{trip.endDate}}`
- `{{trip.itineraryItems[*].date}}`
- `{{trip.itineraryItems[*].activity}}`
- `{{trip.allowedLuggageText}}` (editable manualmente por rol Contratos)

## 3) Viajeros y acompanantes
- `{{travelers.companions[*].fullName}}`
- `{{travelers.companions[*].idType}}`
- `{{travelers.companions[*].idNumber}}`
- `{{travelers.minors[*].fullName}}`
- `{{travelers.minors[*].idNumber}}`
- `{{travelers.minors[*].guardianName}}`
- `{{travelers.hasCompanions}}`
- `{{travelers.hasMinors}}`

## 4) Pago
- `{{payment.totalAmount}}`
- `{{payment.plan}}` (contado/cuotas)
- `{{payment.initialAmount}}` (si cuotas)
- `{{payment.installmentsSummary}}` (si cuotas)
- `{{payment.dueDate}}` (si cuotas)
- `{{payment.paidAt}}` (si contado)

## 5) Hospedaje/logistica
- `{{lodging.checkInDateFrom}}`
- `{{lodging.checkInDateTo}}`
- `{{lodging.checkOutDate}}`

## 6) Condiciones y anexos
- `{{legal.applyExonerationAnnex}}`
- `{{sale.isResale}}`
- `{{minors.guardianSignatureName}}`
- `{{annexes.insuranceAndCoverage.enabled}}`
- `{{annexes.insuranceAndCoverage.version}}`
- `{{annexes.insuranceAndCoverage.signedAt}}`
- `{{annexes.insuranceAndCoverage.cutoffHours}}` (valor esperado: `48`)

## 6.1) Referencia al anexo de seguro
- El detalle por viajero (titular y acompanantes) ya no se imprime en el contrato general.
- Ese detalle vive en el anexo dedicado para permitir firma y ajuste hasta 48h antes del viaje.

## 7) Fechas de firma
- `{{lucitours.signatories.includeEdwin}}` (seleccion manual en rol Contratos)
- `{{lucitours.signatories.includeErick}}` (seleccion manual en rol Contratos)
- `{{signatures.lucitoursEdwinDate}}`
- `{{signatures.lucitoursErickDate}}`
- `{{signatures.clientDate}}`

## Reglas automaticas clave (resumen)
1. Si `client.idType = passport` -> bloque de pasaporte.
2. Si `client.idType = dimex` -> bloque DIMEX.
3. Si `travelers.hasMinors = true` -> activa bloque de menor + Anexo de Autorizacion.
4. Si `travelers.hasCompanions = true` -> lista acompanantes y firmas extra.
5. Si `payment.plan = installments` -> clausula de deposito y cuotas.
6. Si `payment.plan = cash` -> clausula de pago de contado.
7. Si `legal.applyExonerationAnnex = true` -> agrega anexo de exoneracion.
8. Si `sale.isResale = true` -> aplica texto de reventa en check in/operativa.
9. Seleccion de firmante(s) Lucitours -> manual por rol Contratos antes de generar.
10. El seguro se gestiona fuera del contrato general, en anexo independiente.
11. El anexo de seguro puede firmarse/actualizarse hasta 48h antes del viaje.
12. El contrato general solo referencia version y vigencia del anexo de seguro.

## Paso previo de seleccion manual (rol Contratos)
Antes de generar el contrato final, el usuario del rol Contratos debe seleccionar manualmente quien firma por Lucitours.

Opciones UI sugeridas:
1. Dropdown: `Edwin`, `Erick`, `Ambos`.
2. O checkboxes: `includeEdwin`, `includeErick`.

Reglas de validacion:
1. Debe seleccionarse al menos un firmante.
2. Si `includeEdwin = true`, `signatures.lucitoursEdwinDate` es obligatorio.
3. Si `includeErick = true`, `signatures.lucitoursErickDate` es obligatorio.
4. Si ambos estan en `false`, bloquear la generacion del contrato.
